...
def eval():
    drawconfusionmatrix = DrawConfusionMatrix(labels_name=['愤怒', '厌恶', '恐惧', '快乐', '悲伤', '惊讶', '中立'])
    total_test_loss = 0
    total_test_acc = 0
    count = 0
    model.eval()
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("output/mini_xception_159.pth", map_location=device))
    model.to(device)

    # 数据加载
    test_dataset = FER2013(root='./data', split='test', transform=transforms.ToTensor())
